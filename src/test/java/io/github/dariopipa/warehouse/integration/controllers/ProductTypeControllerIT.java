...
class ProductTypeControllerIT {
    ...
    void test_CreateProductType_ShouldSaveToDatabase() {
        ...
        ResponseEntity<Void> response = restTemplate.exchange(
                "/api/v1/product-types", HttpMethod.POST, entity, Void.class);

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.CREATED);

        List<ProductType> allProductTypes = productTypeRepository.findAll();
        Optional<ProductType> savedProductType = allProductTypes.stream()
                .filter(pt -> uniqueName.equals(pt.getName()))
                .findFirst();
    ...
    void test_GetProductTypes_ShouldReturnPagedResults() {
        ...
        ResponseEntity<PaginatedResponse<ProductTypeResponseDTO>> response = restTemplate.exchange(
                "/api/v1/product-types?page=0&size=10&sortBy=name&direction=asc",
                HttpMethod.GET, entity, new ParameterizedTypeReference<PaginatedResponse<ProductTypeResponseDTO>>() {
                });
    ...
    void test_UpdateProductType_ShouldUpdateDatabaseRecord() {
        ...
        ResponseEntity<Void> response = restTemplate.exchange(
                "/api/v1/product-types/" + productTypeId, HttpMethod.PATCH, entity, Void.class);
    ...
    void test_DeleteProductType_ShouldSoftDeleteFromDatabase() {
        ...
        ResponseEntity<Void> response = restTemplate.exchange(
                "/api/v1/product-types/" + productTypeId, HttpMethod.DELETE, entity, Void.class);
    ...
    void test_CreateProductType_WithDuplicateName_ShouldReturnConflictException() {
        ...
        ResponseEntity<String> response = restTemplate.exchange(
                "/api/v1/product-types", HttpMethod.POST, entity, String.class);
    ...
    private String authenticateAndGetToken() {
        ...
        ResponseEntity<JwtResponse> response = restTemplate.postForEntity(
                "/api/v1/auth/login", loginRequest, JwtResponse.class);
